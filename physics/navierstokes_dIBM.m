function [I, J, V, F] = navierstokes_dIBM(mesh, conn, edof, dof, Re, dt, U_k, U_n, f_source)
...
% for full stabilization consistency.
    ...
    for t = 1:length(elem_types)
        ...
        for q = 1:nGP
            ...
            eps_e = 1.0 * h_e;             % Interface dynamically tracks exactly 1 mesh-width
            Kp_e  = 1e-1 * (h_e.^2);       % Darcy permeability naturally scales with element area to prevent solver singularity
            %disp(max(eps_e))
            %disp(max(Kp_e))
